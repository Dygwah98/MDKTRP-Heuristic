...
class GeneticAlgorithmData {
    ...
    public:
        ...
        const unsigned mutator;
        const unsigned crossover;
        const unsigned max_evaluations_GA;
        const unsigned mut_rate;
...
double GeneticAlgorithm(const Test& instance, const Individual& ind) {
    ...
    for (unsigned i = 0; i < gdata.tries; ++i)
    {
        ...
        std::vector<Individual> individuals_ptr = std::vector<Individual>(gdata.population_size, ind);
        ...
        std::vector<Individual> new_generation_ptr = std::vector<Individual>(gdata.population_size, ind);
